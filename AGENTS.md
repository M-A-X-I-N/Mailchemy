...
R0 and R1 are complete: the TypeScript infrastructure/conformance harness is in place, and the first real canonical semantic contracts plus shared A/B/C rule fixtures are implemented. CRH-27 through CRH-43 are complete: Milestone R2 now includes the first Sieve/Purelymail, Gmail, Outlook / Microsoft Graph, and Thunderbird slices, generated cross-IO matrix, canonical-routed cross-codec round trips, negative exactness coverage, and CI regression baseline. CRH-44 is complete: CRH-44A, CRH-44B, the full CRH-44C1–CRH-44C11 exhaustive documentation umbrella, and CRH-44D1–CRH-44D9 have all been closed. [`docs/CODE_DOCUMENTATION_CONTRACT.md`](docs/CODE_DOCUMENTATION_CONTRACT.md) remains normative for TypeScript documentation, and [`docs/PROVISIONAL_SOURCE_ORGANIZATION.md`](docs/PROVISIONAL_SOURCE_ORGANIZATION.md) is the approved CRH-44D naming/tree design. Authored TypeScript filenames use `lower_snake_case`; semantic naming, Core organization, Conformance/Cross-IO organization, adapter-local organization, import/export/documentation reconciliation, the unfamiliar-reader browseability audit, and the behavior-neutral closeout proof are complete; CRH-45 is next. Do not jump ahead into rewrite-planner work outside the active roadmap sequence unless the human explicitly redirects the work.
