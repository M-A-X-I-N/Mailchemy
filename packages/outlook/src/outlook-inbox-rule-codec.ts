...
function inspectConditions(
  ...
  if (!isRecord(conditions)) {
    return {
      kind: "invalid",
      message: "Outlook messageRule conditions must be an object when present.",
    };
  }
  ...
  if (conditions.subjectContains !== undefined) {
    if (
      ...
      conditions.subjectContains.some((value) => typeof value !== "string")
    ) {
      return {
        kind: "invalid",
...
function decodeActionOnly(
  ...
  if (typeof actions.markAsRead !== "boolean") {
    return invalidNative("Outlook markAsRead must be boolean when present.");
  }
